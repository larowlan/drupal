...
class NodeViewTest extends NodeTestBase {
  ...
  public function testHtmlHeadLinks() {
    ...
    $this->assertEqual($result[0]->getAttribute('href'), url("node/{$node->id()}/revisions"));

    $result = $this->xpath('//link[@rel = "edit-form"]');
    $this->assertEqual($result[0]->getAttribute('href'), url("node/{$node->id()}/edit"));

    $result = $this->xpath('//link[@rel = "canonical"]');
    $this->assertEqual($result[0]->getAttribute('href'), url("node/{$node->id()}"));
  }
